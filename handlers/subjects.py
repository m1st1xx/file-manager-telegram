...
def uid(cb): return SESSIONS.get(cb.from_user.id)

@router.callback_query(F.data == "menu")
async def menu(cb: CallbackQuery):
    if not uid(cb): return await cb.answer("Сначала выполните /start", show_alert=True)
...
async def show_subjects(cb: CallbackQuery):
    ...
    if not user: return await cb.answer("Сессия недействительна", show_alert=True)
    mode = "view"
    if cb.data == "download_subjects": mode = "download"
    if cb.data == "upload_subjects": mode = "upload"
    subjects = [r["name"] for r in get_subjects(user["id"])]
    if not subjects:
